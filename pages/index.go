...
func DashboardPage(ctx *context.Context) (types.Panel, error) {
	...
	infobox1 := infobox.New().
		SetText("总卖出数量").
		SetColor(color.Aqua).
		SetNumber(statics.SumSalesTmpl()).
	...
	infobox3 := infobox.New().
		...
		SetNumber(statics.AmountSalesTmpl()).
	...
	infobox4 := infobox.New().
		...
		SetNumber(statics.ToDaySumSalesTmpl()).
	...
	row1 := components.Row().SetContent(infoboxCol1 + infoboxCol3 + infoboxCol4).GetContent()
	...
	tableCol := colComp.SetSize(types.SizeMD(12)).SetContent(row1 + boxInfo).GetContent()
	...
	lineChart := line.
		SetID("salechart").
		SetHeight(180).
		SetTitle("售卖记录: 1 Jan, 2019 - 30 Jul, 2019").
	...
	smallbox1 := smallbox.New().SetColor("blue").SetIcon("ion-ios-gear-outline").SetUrl("/").SetTitle("售出金额").SetValue(statics.AmountSalesTmpl()).GetContent()
	smallbox2 := smallbox.New().SetColor("yellow").SetIcon("ion-ios-cart-outline").SetUrl("/").SetTitle("购买人数").SetValue(statics.UserSumTmpl()).GetContent()
	smallbox3 := smallbox.New().SetColor("red").SetIcon("fa-user").SetUrl("/").SetTitle("卖出盲盒").SetValue(statics.SumSalesTmpl()).GetContent()
	smallbox4 := smallbox.New().SetColor("green").SetIcon("ion-ios-cart-outline").SetUrl("/").SetTitle("今天卖出金额").SetValue(statics.TodayAmountSalesTmpl()).GetContent()
